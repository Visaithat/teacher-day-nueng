...
import { HIT_RATIO, TIMING } from "./mailbox-data";
...
export function useKeyDrag() {
  ...
  const centreOf = (node: Element | null) => {
  ...
  const unlock = useCallback((offsetX: number, offsetY: number) => {
    ...
    setState("seating");
    timers.current.push(
      window.setTimeout(() => setState("turning"), TIMING.seat),
      window.setTimeout(() => {
        setState("open");
        setAnnouncement(
          "The mail box is open. There is a present inside, ready to open.",
        );
      }, TIMING.seat + TIMING.turn),
    );
  }, []);
  ...
  const springBack = useCallback(() => {
    setOffset(0, 0);
    setState("returning");
    timers.current.push(window.setTimeout(() => setState("idle"), 340));
  }, []);
